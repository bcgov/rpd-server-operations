...
token <- base64enc::base64encode(charToRaw(paste0(username, ":", credentials)))
...
entities <- resp |>
  ...
  tidyr::unnest_wider(dplyr::where(is.list), names_sep = "-") |>
  dplyr::select(!(tidyr::ends_with("datatype"))) |>
