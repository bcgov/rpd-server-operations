...
# This below will need refining if we want to keep historical records with our weird internal processes
# tryCatch(
#   {
...
# AND (tgt.rmpct_ls_id = src.rmpct_ls_id
#      OR (tgt.rmpct_ls_id IS NULL AND src.rmpct_ls_id IS NULL))
# AND (tgt.rmpct_dv_id = src.rmpct_dv_id
#      OR (tgt.rmpct_dv_id IS NULL AND src.rmpct_dv_id IS NULL))
# AND (tgt.rmpct_dp_id = src.rmpct_dp_id
#      OR (tgt.rmpct_dp_id IS NULL AND src.rmpct_dp_id IS NULL))
...
# Identity Stability Test - use this if ever going for a historical preservation
#
# query <- dbSendQuery(
#   con,
#   paste0(
