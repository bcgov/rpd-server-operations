...
TABLE_NAME <- "kahua_project_role"
CBRE_TABLE_NAME <- "kahua_project_role"
TARGET_TABLE <- DBI::Id(schema = SCHEMA_NAME, table = TABLE_NAME)
TEMP_TABLE <- paste0("#", TABLE_NAME, "Temp")
API_NAME <- "CBRE"
SCRIPT_NAME <- "kahua_project_role"
...
if (!dbExistsTable(con, TARGET_TABLE)) {
  sql <- paste0(
    ...
    " (
      ...
      project_role_alias_name NVARCHAR(100)  NOT NULL,
...
tryCatch(
  {
    ...
    if (dup_count > 0) {
      stop(paste0(
        "Duplicate project_role values detected in source data (",
        dup_count,
      ...
      paste0(
        "UPDATE tgt
         SET
           ...
           tgt.project_role_alias_name = src.project_role_alias_name,
        ...
        TEMP_TABLE,
        " src
         ON tgt.project_role = src.project_role;"
      )
    )
    ...
    n_inserted <- dbExecute(
      ...
      LEFT JOIN ",
        ...
        " tgt
      ON  tgt.project_role = src.project_role
      WHERE tgt.project_role IS NULL;
      "
      )
    )
