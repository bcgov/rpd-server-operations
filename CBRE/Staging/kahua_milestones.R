...
TABLE_NAME <- "kahua_milestones"
CBRE_TABLE_NAME <- "kahua_milestones"
TARGET_TABLE <- DBI::Id(schema = SCHEMA_NAME, table = TABLE_NAME)
TEMP_TABLE <- paste0("#", TABLE_NAME, "Temp")
API_NAME <- "CBRE"
SCRIPT_NAME <- "kahua_milestones"
...
clean_data <- raw_data |>
  ...
  select(
    RefreshDate,
    milestone_name,
    milestone_alias_name,
    activity_skey,
    edp_source_system_code,
    edp_create_ts,
    edp_update_ts
  )
...
if (!dbExistsTable(con, TARGET_TABLE)) {
  sql <- paste0(
    ...
    " (
      ...
      milestone_alias_name    NVARCHAR(100)  NOT NULL,
      activity_skey           NVARCHAR(10)   NOT NULL,
      edp_source_system_code  NVARCHAR(10)   NOT NULL,
...
tryCatch(
  {
    ...
    if (dup_count > 0) {
      stop(paste0(
        "Duplicate milestone_name values detected in source data (",
        dup_count,
      ...
      paste0(
        "UPDATE tgt
         SET
           ...
           tgt.milestone_name          = src.milestone_name,
           tgt.milestone_alias_name    = src.milestone_alias_name,
           tgt.activity_skey           = src.activity_skey,
           tgt.edp_source_system_code  = src.edp_source_system_code,
        ...
        TEMP_TABLE,
        " src
         ON tgt.milestone_name = src.milestone_name;"
      )
    )
    ...
    n_inserted <- dbExecute(
      ...
      SELECT
        src.RefreshDate,
        src.milestone_name,
        src.milestone_alias_name,
        src.activity_skey,
        src.edp_source_system_code,
        src.edp_create_ts,
        src.edp_update_ts
      FROM ",
        TEMP_TABLE,
      ...
      LEFT JOIN ",
        ...
        " tgt
      ON  tgt.milestone_name = src.milestone_name
      WHERE tgt.milestone_name IS NULL;
      "
      )
    )
